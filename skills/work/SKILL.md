...
- **After each step completes** (including intake and checkpoint approvals), append: `step {N} — {agent name(s)} — complete — {output path(s)}`. For intake: `intake — complete — .loom/artifacts/{ticket_id}/intake-brief.md`. Record worktree-relative paths (e.g., `.loom/artifacts/{ticket_id}/research.md`) — beyond resumability, the progress file is the canonical ledger of work-phase artifacts: the review phase resolves its upstream from these records (artifact paths are never written to the ticket; see shared/backlog.md § References). Loop steps (convergence, cross-talk) are recorded only on full completion — a partially finished loop restarts from round 1 on resume.
...
5. If complete: no backlog call — the step's progress record (see Progress tracking) is the artifact ledger the review phase consumes. Exception: convergence and cross-talk steps follow their own recording rules (shared/convergence.md, shared/cross-talk.md) — only final-round outputs are recorded, so the ledger carries standing verdicts, not intermediate rounds.
