...
- Focus on three questions: (1) Does the fix introduce more complexity than the bug warrants? (2) Could standard library or framework features replace custom code? (3) Did iterative fix rounds accumulate unnecessary changes beyond the bug's scope?
...
- A correct but unnecessarily complex fix is `should-fix`, not `must-fix`. Reserve `must-fix` for cases where complexity introduces a maintenance or correctness risk.

## Evaluation criteria

- Unnecessary abstraction layers
- Custom code that duplicates framework features
- Files modified beyond the bug's scope
- Verbose error handling for impossible cases
- Dead code introduced by fix iterations
