...
- Use `git diff --stat {default_branch}` for the diff overview — read `default_branch` from `## config`. Read full diff only for files mentioned in reviewer findings.
...
3. Run `git diff --stat {default_branch}` in the worktree for the diff overview.
