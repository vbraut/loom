...
- Use `git -C {worktree_path} diff --stat {default_branch}` for the diff overview. Read full diff only for files mentioned in reviewer findings.
...
3. Run `git -C {worktree_path} diff --stat {default_branch}` for the diff overview.
