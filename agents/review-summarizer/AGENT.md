...
- Use `git diff --stat` for the diff overview. Read full diff only for files mentioned in reviewer findings.
...
3. Run `git diff --stat` in the worktree for the diff overview.
