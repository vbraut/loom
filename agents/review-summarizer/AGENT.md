...
- Use `git -C {worktree_path} diff --stat {sync_ref}` for the diff overview. Read full diff only for files mentioned in reviewer findings.
...
3. Run `git -C {worktree_path} diff --stat {sync_ref}` for the diff overview.
