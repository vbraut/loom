...
- Review the actual worktree diff (`git diff {default_branch}...HEAD` — read `default_branch` from `## config`). Only audit files that contain UI code (templates, components, styles).
