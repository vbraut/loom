...
3. **Spawn reviewer agents.** For agents marked `(parallel)`, spawn all via multiple Agent tool calls in a single response. Resolve path placeholders: replace `{ticket_id}` with the current ticket ID and `{N}` with the current `round` value. All paths must be absolute (resolved from the worktree root). Include the **Upstream for reviewers** paths (parsed in step 1) as the base `## upstream_artifacts` for each reviewer in every round. Include a `## panel` section listing the other reviewers running in this step — agents with dual standalone/panel behavior (e.g., adversarial-reviewer) use it to avoid duplicating specialist findings. Every reviewer prompt MUST also carry an explicit output instruction: the reviewer is required to call the Write tool to persist its full assessment (findings, summary, and the `STATUS:`/`VERDICT:` line) to its resolved `## output_path` **before responding** — its inline response is read only for the verdict, so an inline-only answer leaves no artifact for the exit sequence to record. The reviewer AGENT.md "Output" directive is too soft to guarantee this on its own — several reviewers skip the Write on a clean (no-findings) pass — so this prompt-level instruction is mandatory and step 4c is the backstop. (Cross-talk messages already carry the equivalent directive, which is why cross-talk agents persist reliably.) When the convergence target is a plan or spec (not code), append this note to every reviewer prompt: "This is a plan review. Plans describe architecture — what to change and why. Do not flag missing function signatures, mock body shapes, assertion syntax, payload schemas, or other code-level implementation details. Those are the implement agent's responsibility and will be caught during code convergence."
...
4c. **Verify each reviewer persisted its output.** Confirm every reviewer's resolved `## output_path` exists and holds at least one non-whitespace character. The step-4 verdict is read from the response, but the file is the artifact the exit sequence records and the review phase resolves upstream from — a reviewer that answered inline without calling Write leaves none. For any reviewer whose file is missing or whitespace-only, re-spawn it once (same prompt, with the explicit Write instruction from step 3); if the file is still missing or empty after the re-spawn, treat that reviewer as `STATUS: failed — reviewer did not persist output_path`. This check runs every round, on both the clean-pass and needs-work paths, so a clean final round can never record a non-existent artifact (the historical failure mode this guards against: reviewers returning a `pass` verdict inline while skipping the file, which the old step-8-only check missed whenever a round passed).
...
1. **Record final outputs.** List in this step's progress record (see the work skill's Progress tracking): each reviewer output_path from the final round (guaranteed present and non-empty by step 4c), plus every round's feedback agent output_path. No backlog call — the progress record is the artifact ledger the review phase resolves upstream from; recording only final-round reviewer outputs keeps intermediate rounds out of downstream consumption.
